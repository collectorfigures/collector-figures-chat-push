...
The public repository is the authoritative Corresponding Source for each Collector Figures Web Push Gateway production image.

Required GitHub rules for `main`:

- deny force-push and branch deletion;
- require linear history;
- require the `CFS Push source verification / verify` status check;
- require conversations to be resolved before merge;
- block merge while the required check is stale or failing.

Release rules:

- production builds run only from an immutable `cfs-push-v*` tag;
