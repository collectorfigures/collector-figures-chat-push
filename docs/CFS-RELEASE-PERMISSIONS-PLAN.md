...
- Repository ruleset `Protect cfs-push release tags` targets `refs/tags/cfs-push-v*`.
...
Keep the existing update/deletion ruleset unchanged and add a separate creation-only ruleset for `refs/tags/cfs-push-v*`:
...
Create a protected GitHub Environment named `cfs-push-release` before enabling the release workflow:
...
- deployment branch/tag policy limited to `cfs-push-v*`;
...
The Draft Release Workflow now declares the exact source-level binding `environment.name: cfs-push-release`. This is a fail-closed prerequisite only: the Environment, reviewer policy, Team, creation Ruleset, and permissions have not been created or changed. They must be created and independently verified before this Draft PR may be merged.
