# CFS Push release permission plan
...
- `Protect cfs-push release tags` targeted `refs/tags/cfs-push-v*` and blocked update and deletion with no bypass actor.
...
The Push release gate is:

- Environment: `cfs-push-release`;
...
- sole deployment tag policy: `cfs-push-v*`;
- Environment secrets: `0`;
- Environment variables: `0`;
- creation-only Ruleset: `Restrict cfs-push release tag creation` / ID `22177550`;
- creation bypass: Team ID `19325995` only, mode `always`;
- unchanged update/deletion Ruleset: `Protect cfs-push release tags` / ID `21900094`;
- update/deletion bypass actors: `0`.

The Draft Release Workflow declares `environment.name: cfs-push-release`. Packages write and id-token write remain confined to the Environment-protected release job; the preceding tag-validation job has only `contents: read`.

## Formal release tag admission

The GitHub `cfs-push-v*` pattern is only a coarse namespace filter. The authoritative Formal Release admission is the Runtime exact regex:

```text
^cfs-push-v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$
```
